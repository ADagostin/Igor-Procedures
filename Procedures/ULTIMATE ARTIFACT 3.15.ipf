...
//if (stringmatch(S_Value,"No Graphs Available"))
//	dowindow
//endif
...
execute "Killwaves /z Template, Template_ori, Review, Prev_template" 
End
...
Wave  Amplitudes_Time, Areas, Output_Data, Amplitudes_Norm
	...
	for (i=0;i<itemsinlist(list);i+=1)
		Art_Ult(Stringfromlist(i,list),Graph,Check)	
//		Fits(i)
...
Variable Artifact_Found,j,Manual, Manual_init
String Name
...
//code to avoid very simple artifact waveform not to be positively correlaterd to baseline noise.!!!!!! ex: cell#2 012519
		...
		Else
			...
			Manual_init=x2pnt(test,Stim_Timepoints[index])
			index+=1
		Endif
 		If (Artifact_Found>0)
...
//	Absolute_Baseline=V_Value
	...
	if (Absolute==-1)
		...
		If (V_Value==0)
			...
			Controlinfo /W=Artifact_Handling Baseline
			if (V_Value)
				i=get_i(Absolute,i,x2pnt(test,V_minloc),test)
			else
			...
			Wavestats /q /r=(Stim_Timepoints[index]+abs(xcsr(a)-xcsr(b)),Stim_Timepoints[index]+(Stim_Timepoints[1]-Stim_Timepoints[0])) test//[The_End,The_End+Variation_DeltaX] Test
			Manual_Baseline=Stim_Timepoints[index]+abs(xcsr(a)-xcsr(b))
		endif			
		if (k==0)
			Amplitudes[k]=V_min
			controlinfo /W=Artifact_Handling Manual_Freq_Input
		...
		If (V_Value)
			...
			Baseline[k]=Test(Stim_Timepoints[index])
		else
			Amplitudes[k]=V_min-Test[i]
			Baseline[k]=Test[i]
		endif
		...
		If (V_Value==0)
			...
			If (V_Value)
				Amplitudes[k]=V_max//-Test[i]
				Baseline[k]=Test(Manual_Baseline)
			else
				Amplitudes[k]=V_max-Test[i]
				Baseline[k]=Test[i]
			endif
...
Static function get_i(Absolute,P1,P2,wv_name)
variable Absolute,P1,p2
wave Wv_name
If (absolute==1)
findpeak /q /P /n /r=[P1+3,P1+250] Wv_name
else
findpeak /q /P /r=[P1+3,P1+250] Wv_name
//wavestats /p /q/r=[P1+3,p2] Wv_Name
endif
return v_Peakloc//maxloc
end
...
for (i=0;i<itemsinlist(Wvs);i+=1)
	if (multiple_waves==1)
		...
		if (waveexists($stringfromlist(i,Wvs)))
...
Function Slid_Avg(Start_Pnt,SourceWave)
	...
	if ((V_max-V_min)>(wavemax(Prev_Template)-Wavemin(Prev_Template))/8)	
		...
		if (Start_Pnt<(numpnts(SourceWave)-(Template_Size*2-2)))
			...
			wavestats /q res
			//print v_max
			if (V_Max>Tolerance)// || statscorrelation(template, prev_template)>=tolerance)// && Prev_Template_Amp>=Template_Amp/10)			
				Return Look_Forward(Start_Pnt, SourceWave, Template,Template_Size, V_Max)+V_Maxrowloc		
			//return Start_pnt+V_Maxrowloc
			else	
				//print "V_max < Tolerance - line 968"	, Start_Pnt
				return 0
			endif			
		Else	
			//print "2nd return"	
		...
		//print "main if statement return",Start_Pnt
		return 0
endif
End
